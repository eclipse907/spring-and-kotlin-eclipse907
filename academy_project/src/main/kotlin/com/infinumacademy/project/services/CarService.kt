...
import com.infinumacademy.project.exceptions.*
...
class CarService(
    ...
    fun addCar(carToAdd: AddCarDto): CarDto {
        if (carToAdd.dateAdded > LocalDate.now()) {
            throw WrongCarDateAddedException("Car added date can't be after current date")
        }
        if (carToAdd.productionYear > Year.now().toString().toShort()) {
            throw WrongCarProductionYearException("Car production year can't be after current year")
        }
